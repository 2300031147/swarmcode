...
use swarm_plugins::{PluginManager, PluginManagerConfig};
...
use swarm_runtime::{
    ...
    Session, SwarmHive, HiveMember, HiveMemberStatus, HiveRole, TokenUsage, ToolError, ToolExecutor,
    UsageTracker,
};
use serde_json::json;
use swarm_tools::{GlobalToolRegistry, init_global_team_hub, init_global_team_hub_with_id};
use dialoguer::{theme::ConfirmTheme, Select};
use console::{style, Term};

const DEFAULT_MODEL: &str = "swarm-model-opus";
fn max_tokens_for_model(model: &str) -> u32 {
    if model.contains("swarm-opus") {
        32_000
    } else {
        64_000
    }
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Teammate { team_id, agent_id, model, permission_mode } => {
            run_teammate(&team_id, &agent_id, &model, permission_mode)?
        }
    }
    Ok(())
}
...
fn run_teammate(
    ...
    let hub = swarm_tools::init_global_team_hub_with_id(team_id);
    
    // Find the task manifest
    let manifest_path = swarm_tools::agent_store_dir()?.join(format!("{}.json", agent_id));
    let manifest_content = fs::read_to_string(&manifest_path)?;
    let manifest: swarm_tools::AgentOutput = serde_json::from_str(&manifest_content)?;

    // Standard sub-agent prompts/tools
    let subagent_type = manifest.subagent_type.as_deref().unwrap_or("Explore");
    let system_prompt = swarm_tools::build_agent_system_prompt(subagent_type)?;
    let allowed_tools = swarm_tools::allowed_tools_for_subagent(subagent_type);

    // Initialise the CLI runtime with the team hub
    let mut runtime = build_runtime(
        ...
        allowed_tools,
        permission_mode,
        None, // progress reporter
    )?;

    // Link to team hub
    swarm_tools::set_current_agent_id(agent_id.to_string());
    runtime = runtime.with_team(hub, agent_id.to_string());

    println!("Teammate agent '{}' (ID: {}) started.", manifest.name, agent_id);
    println!("Type: {}, Team: {}", subagent_type, team_id);

    // Initial prompt from manifest (found in the .md file)
    let task_path = swarm_tools::agent_store_dir()?.join(format!("{}.md", agent_id));
    let task_content = fs::read_to_string(&task_path)?;
    let prompt = task_content.split("## Prompt").nth(1).unwrap_or(&task_content).trim();

    let mut permission_prompter = CliPermissionPrompter::new(permission_mode);
    if let Err(error) = runtime.run_turn(prompt, Some(&mut permission_prompter)) {
        let _ = hub.set_status(&agent_id, HiveMemberStatus::Failed);
        return Err(error.into());
    }

    let _ = hub.set_status(agent_id, HiveMemberStatus::Completed);
    println!("Teammate agent '{}' finished.", manifest.name);
    
    // Lifecycle Polish: unregister on exit
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--team" => {
                team_id = Some(args.get(index + 1).ok_or("missing --team value")?.clone());
                index += 2;
            }
            "--agent-id" => {
                agent_id = Some(args.get(index + 1).ok_or("missing --agent-id value")?.clone());
                index += 2;
            }
            "--version" | "-V" => {
                wants_version = true;
            ...
            "-p" => {
                // ClawSwarm compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // ClawSwarm compat: --print makes output non-interactive
    ...
    if let (Some(tid), Some(aid)) = (team_id, agent_id) {
        return Ok(CliAction::Teammate {
            team_id: tid,
            agent_id: aid,
            model,
            permission_mode,
...
fn print_bootstrap_plan() {
    for phase in swarm_runtime::BootstrapPlan::claw_default().phases() {
        println!("- {phase:?}");
    }
}
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        scopes: vec![
            ...
            String::from("user:sessions:clawswarm"),
        ],
    }
}
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let redirect_uri = swarm_runtime::loopback_redirect_uri(callback_port);
...
fn wait_for_oauth_callback(
    port: u16,
) -> Result<swarm_runtime::OAuthCallbackParams, Box<dyn std::error::Error>> {
...
Aliases
  opus             swarm-model-opus
  sonnet           swarm-model-sonnet
  haiku            swarm-model-haiku-4-5-20251213
...
fn run_resume_command(
    ...
    match command {
        ...
        SlashCommand::Compact => {
            let result = swarm_runtime::compact_session(
                session,
                CompactionConfig {
...
fn run_repl(
    ...
    loop {
        match editor.read_line()? {
            ...
            input::ReadOutcome::SwitchAgent => {
                if let Some(hub) = &cli.team_hub {
                    if let Some(choice) = cli.show_agent_switcher(hub) {
                        cli.sessions.switch_to(&choice);
                        println!("Switched to agent: {}", choice);
                    }
                }
            }
            input::ReadOutcome::CycleAgent => {
                cli.sessions.cycle_next();
                println!("Active agent: {}", cli.sessions.active_agent_id);
            }
...
struct AgentSessionManager {
    sessions: BTreeMap<String, AgentSession>,
    active_agent_id: String,
}

impl AgentSessionManager {
    fn new(lead_id: String, lead_runtime: ConversationRuntime<DefaultRuntimeClient, CliToolExecutor>) -> Self {
        let mut sessions = BTreeMap::new();
        sessions.insert(lead_id.clone(), AgentSession {
            id: lead_id.clone(),
            runtime: lead_runtime,
        });
        Self {
            sessions,
            active_agent_id: lead_id,
        }
    }

    fn cycle_next(&mut self) {
        let ids: Vec<String> = self.sessions.keys().cloned().collect();
        if let Some(pos) = ids.iter().position(|id| id == &self.active_agent_id) {
            let next_pos = (pos + 1) % ids.len();
            self.active_agent_id = ids[next_pos].clone();
        }
    }

    fn active_session_mut(&mut self) -> &mut AgentSession {
        self.sessions.get_mut(&self.active_agent_id).expect("Active session gone")
    }

    fn switch_to(&mut self, agent_id: &str) -> bool {
    ...
    fn add_session(&mut self, id: String, runtime: ConversationRuntime<DefaultRuntimeClient, CliToolExecutor>) {
        self.sessions.insert(id.clone(), AgentSession { id, runtime });
    }
}
...
impl LiveCli {
    fn new(
        ...
        // ── Agent Teams: optionally initialise the team hub ──────
        let team_hub = if std::env::var("CLAWSWARM_AGENT_TEAMS")
            .ok()
            .map_or(false, |v| matches!(v.trim(), "1" | "true" | "yes"))
        {
            let hub = init_global_team_hub();
            hub.register(HiveMember {
                id: "lead".to_string(),
                name: "lead".to_string(),
                description: "Team lead agent".to_string(),
                role: HiveRole::Lead,
                status: HiveMemberStatus::Active,
            });
            runtime = runtime.with_team(Arc::clone(&hub), "lead".to_string());
            Some(hub)
        } else {
            None
        };
        ...
        let cli = Self {
            ...
            permission_mode,
            system_prompt,
            sessions: AgentSessionManager::new(String::from("lead"), runtime),
            session,
            team_hub,
        };
        cli.persist_session()?;
    ...
    fn show_agent_switcher(&self, hub: &SwarmHive) -> Option<String> {
        let members = hub.members();
        if members.is_empty() { return None; }

        println!("\n  --- Select Agent ---");
        for (i, m) in members.iter().enumerate() {
            let active_mark = if m.id == self.sessions.active_agent_id { "*" } else { " " };
            println!("  [{}] {active_mark} {} - {}", i, m.id, m.description);
        }
        print!("  Selection (or Enter to cancel): ");
        io::stdout().flush().ok();
        ...
        if io::stdin().read_line(&mut input).is_ok() {
            ...
            if let Ok(idx) = trimmed.parse::<usize>() {
                if idx < members.len() {
                    return Some(members[idx].id.clone());
                }
            }
        }
        None
    ...
    fn startup_banner(&self) -> String {
        ...
        let mut lines = vec![
            format!(
                "{} {}",
                if color {
                    "\x1b[1;38;5;45m🦞 ClawSwarm\x1b[0m"
                } else {
                    "ClawSwarm"
                },
                if color {
            ...
            format!(
                "  {} {}",
                if color { "\x1b[38;5;214mID\x1b[0m" } else { "ID" },
                self.session.id
            ),
            format!(
                "  {} {}",
                if color { "\x1b[38;5;118mAgent\x1b[0m" } else { "Agent" },
                self.sessions.active_agent_id
            ),
            format!(
                "  {} {}",
                if color { "\x1b[38;5;208mPath\x1b[0m" } else { "Path" },
                cwd_display
            ),
        ...
        if self.team_hub.is_some() {
            lines.push(
                "  Agent Teams      \x1b[1;32menabled\x1b[0m · /team to view teammates".to_string(),
            );
        }
        lines.join("\n")
    }
    ...
    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result = self.sessions.active_session_mut().runtime.run_turn(input, Some(&mut permission_prompter));
        match result {
            Ok(_) => {
                spinner.finish(
                    "✨ Done",
    ...
    fn run_prompt_json(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        let session = self.sessions.active_session_mut().runtime.session().clone();
        let mut runtime = build_runtime(
            session,
            self.model.clone(),
        ...
        self.sessions.active_session_mut().runtime = runtime;
        self.persist_session()?;
        println!(
            "{}",
            json!({
    ...
    fn show_agent_switcher(&self, hub: &Arc<SwarmHive>) -> Option<String> {
        let members = hub.members();
        if members.is_empty() {
            println!("{}", style("No other agents in the team.").yellow());
            return None;
        }
        ...
        // Add the lead first if not already there, or just list everyone
        for member in members.values() {
            let role_label = match member.role {
                HiveRole::Lead => style(" (Lead)").cyan(),
                HiveRole::Teammate => style(" (Teammate)").dim(),
            };
            let status_label = match member.status {
                HiveMemberStatus::Active => style("●").green(),
                HiveMemberStatus::Idle => style("○").dim(),
                HiveMemberStatus::Completed => style("✓").green(),
                HiveMemberStatus::Failed => style("✗").red(),
            };
            items.push(format!("{} {} {}{}", status_label, style(&member.id).bold(), member.name, role_label));
            ids.push(member.id.clone());
        }

        let selection = Select::with_theme(&ConfirmTheme::default())
            .with_prompt("Select an agent to switch to")
            .items(&items)
            .default(0)
    ...
    fn print_team_status(&self) {
        let Some(hub) = &self.team_hub else {
            println!("{}", style("Agent Teams is not enabled for this session.").red());
            return;
        };

        let team_id = hub.team_id().unwrap_or_else(|| "local".to_string());
        println!("\n{}", style(format!("Team: {}", team_id)).bold().underlined());
        ...
        for member in members.values() {
            ...
            let status_label = match member.status {
                HiveMemberStatus::Active => style("ACTIVE").green(),
                HiveMemberStatus::Idle => style("IDLE").dim(),
                HiveMemberStatus::Completed => style("COMPLETED").green(),
                HiveMemberStatus::Failed => style("FAILED").red(),
            };
            println!(
                "  {} {:<12} {:<10} {}",
                style(&member.id).bold(),
                role_label,
                status_label,
                member.name
            );
    ...
    fn print_team_status(&self) {
        if let Some(hub) = &self.team_hub {
            let members = hub.members();
            if members.is_empty() {
                println!("\n  Agent Teams Feature is enabled, but no agents are registered.\n");
                return;
            }
            let mut lines = vec![String::from("\n  Agent Team Status:")];
            for member in members {
                let status_icon = match member.status {
                    HiveMemberStatus::Active => "🟢",
                    HiveMemberStatus::Idle => "⚪",
                    HiveMemberStatus::Completed => "✅",
                    HiveMemberStatus::Failed => "❌",
                };
                let role_label = match member.role {
                    HiveRole::Lead => "Lead",
                    HiveRole::Teammate => "Teammate",
                };
                lines.push(format!(
                    "    {status_icon} {} ({}) - {}",
                    member.id, role_label, member.description
                ));
            }
            println!("{}\n", lines.join("\n"));
        } else {
            println!("\n  Agent Teams is disabled. Enable with CLAWSWARM_AGENT_TEAMS=true\n");
        }
    }
...
fn build_plugin_manager(
    cwd: &Path,
    loader: &ConfigLoader,
    runtime_config: &swarm_runtime::RuntimeConfig,
) -> PluginManager {
...
impl swarm_runtime::PermissionPrompter for CliPermissionPrompter {
    fn decide(
        &mut self,
        request: &swarm_runtime::PermissionRequest,
    ) -> swarm_runtime::PermissionPromptDecision {
        println!();
        ...
        match io::stdin().read_line(&mut response) {
            Ok(_) => {
                ...
                if matches!(normalized.as_str(), "y" | "yes") {
                    swarm_runtime::PermissionPromptDecision::Allow
                } else {
                    swarm_runtime::PermissionPromptDecision::Deny {
                        reason: format!(
            ...
            Err(error) => swarm_runtime::PermissionPromptDecision::Deny {
...
fn resolve_cli_auth_source() -> Result<AuthSource, Box<dyn std::error::Error>> {
    Ok(resolve_startup_auth_source(|| {
        let cwd = env::current_dir().map_err(swarm_api::ApiError::from)?;
        let config = ConfigLoader::default_for(&cwd).load().map_err(|error| {
            swarm_api::ApiError::Auth(format!("failed to load runtime OAuth config: {error}"))
        })?;
...
fn collect_tool_uses(summary: &swarm_runtime::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .assistant_messages
        .iter()
...
fn collect_tool_results(summary: &swarm_runtime::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .tool_results
        .iter()
        .flat_map(|message| message.blocks.iter())
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "ClawSwarm CLI v{VERSION}")?;
    writeln!(
        out,
...
mod tests {
    ...
    use swarm_api::{MessageResponse, OutputContentBlock, Usage};
    use swarm_plugins::{PluginTool, PluginToolDefinition, PluginToolPermission};
    use swarm_runtime::{AssistantEvent, ContentBlock, ConversationMessage, MessageRole, PermissionMode};
    use serde_json::json;
    use std::path::PathBuf;
    use std::time::Duration;
    use swarm_tools::GlobalToolRegistry;
    ...
    fn resolves_model_aliases_in_args() {
        ...
        assert_eq!(
            ...
            CliAction::Prompt {
                prompt: "explain this".to_string(),
                model: "swarm-model-opus".to_string(),
                output_format: CliOutputFormat::Text,
                allowed_tools: None,
                permission_mode: PermissionMode::DangerFullAccess,
            }
    ...
    fn shared_help_uses_resume_annotation_copy() {
        let help = swarm_commands::render_slash_command_help();
    ...
    fn cost_report_uses_sectioned_layout() {
        let report = format_cost_report(swarm_runtime::TokenUsage {
            input_tokens: 20,
            output_tokens: 8,
    ...
    fn model_report_uses_sectioned_layout() {
        let report = format_model_report("swarm-standard", 12, 4);
    ...
    fn model_switch_report_preserves_context_summary() {
        let report = format_model_switch_report("swarm-standard", "swarm-opus", 9);
    ...
    fn status_line_reports_model_and_token_totals() {
        let status = format_status_report(
            ...
            StatusUsage {
                message_count: 7,
                turns: 3,
                latest: swarm_runtime::TokenUsage {
                    input_tokens: 5,
                    output_tokens: 4,
                    cache_creation_input_tokens: 1,
                    cache_read_input_tokens: 0,
                },
                cumulative: swarm_runtime::TokenUsage {
                    input_tokens: 20,
                    output_tokens: 8,
