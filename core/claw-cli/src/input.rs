    ...
    pub fn read_line(&mut self) -> io::Result<ReadOutcome> {
        ...
        loop {
            ...
            match self.handle_key_event(&mut session, key) {
                ...
                KeyAction::SwitchAgent => {
                    session.finalize_render(&mut stdout, &self.prompt, self.vim_enabled)?;
                    return Ok(ReadOutcome::SwitchAgent);
                }
                KeyAction::CycleAgent => {
                    session.finalize_render(&mut stdout, &self.prompt, self.vim_enabled)?;
                    return Ok(ReadOutcome::CycleAgent);
                }
            }
        }
    }
        ...
        match key.code {
            ...
            KeyCode::Down => {
                if key.modifiers.contains(KeyModifiers::SHIFT) {
                    return KeyAction::CycleAgent;
                }
                if !session.has_input() {
                    return KeyAction::SwitchAgent;
                }
