...
use swarm_runtime::{ConversationClient, ConversationMessage, RuntimeError, StreamEvent, UsageSummary};
...
impl CliApp {
    ...
    pub fn run_repl(&mut self) -> io::Result<()> {
        ...
        println!("ClawSwarm interactive mode");
    ...
    fn write_turn_output(
        &self,
        summary: &swarm_runtime::TurnSummary,
        out: &mut impl Write,
    ) -> io::Result<()> {
...
mod tests {
    ...
    fn session_state_tracks_config_values() {
        let config = SessionConfig {
            model: "swarm-standard".into(),
            permission_mode: PermissionMode::DangerFullAccess,
            config: Some(PathBuf::from("settings.toml")),
            output_format: OutputFormat::Text,
        };

        assert_eq!(config.model, "swarm-standard");
