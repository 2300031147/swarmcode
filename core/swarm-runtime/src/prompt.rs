...
use swarm_lsp::LspContextEnrichment;
...
pub const SWARM_MASTER_MODEL_NAME: &str = "SwarmMaster-Pro";
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No ClawSwarm settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
",
    ...
    #[test]
    fn renders_CLAWSWARM_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".claw")).expect("claw dir");
