...
impl ModelPricing {
    #[must_use]
    pub const fn default_standard_tier() -> Self {
        Self {
...
pub fn pricing_for_model(model: &str) -> Option<ModelPricing> {
    ...
    if normalized.contains("swarm-opus") {
        return Some(ModelPricing {
...
impl TokenUsage {
    ...
    pub fn estimate_cost_usd(self) -> UsageCostEstimate {
        self.estimate_cost_usd_with_pricing(ModelPricing::default_standard_tier())
    }
...
impl UsageTracker {
    ...
    pub fn record(&mut self, usage: TokenUsage, hub_sync: Option<(&crate::hive::SwarmHive, &str)>) {
        ...
        if let Some((hub, agent_id)) = hub_sync {
            hub.record_usage(agent_id, usage);
        }
    }
...
mod tests {
    ...
    fn computes_cost_summary_lines() {
        ...
        let lines = usage.summary_lines_for_model("usage", Some("swarm-model-sonnet"));
        assert!(lines[0].contains("estimated_cost=$54.6750"));
        assert!(lines[0].contains("model=swarm-model-sonnet"));
    ...
    fn supports_model_specific_pricing() {
        ...
        let haiku = pricing_for_model("swarm-model-haiku").expect("haiku pricing");
        let opus = pricing_for_model("swarm-model-opus").expect("opus pricing");
