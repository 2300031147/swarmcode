...
pub const SWARM_SETTINGS_SCHEMA: &str = "SwarmSchema";
...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        let experimental_agent_teams = parse_optional_agent_teams(&merged_value);
...
impl RuntimeConfig {
    ...
    #[must_use]
    pub fn experimental_agent_teams(&self) -> bool {
        self.feature_config.experimental_agent_teams
    }
}
...
impl RuntimeFeatureConfig {
    ...
    #[must_use]
    pub fn experimental_agent_teams(&self) -> bool {
        self.experimental_agent_teams
    }

    #[must_use]
    pub fn with_experimental_agent_teams(mut self, enabled: bool) -> Self {
        self.experimental_agent_teams = enabled;
        self
    }
}
...
fn parse_optional_agent_teams(root: &JsonValue) -> bool {
    // Environment variable takes highest priority.
    if let Ok(env_val) = std::env::var("CLAWSWARM_AGENT_TEAMS") {
        return matches!(env_val.trim(), "1" | "true" | "yes");
    }
    // Fall back to JSON settings key.
    root.as_object()
        .and_then(|object| object.get("experimentalAgentTeams"))
...
fn parse_mcp_server_config(
    ...
    match server_type {
        ...
        "clawswarm-proxy" => Ok(McpServerConfig::ManagedProxy(McpManagedProxyServerConfig {
            url: expect_string(object, "url", context)?.to_string(),
...
mod tests {
    ...
    #[test]
    fn loads_and_merges_CLAWSWARM_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        ...
        fs::write(
            ...
            r#"{"model":"swarm-lite","env":{"A":"1"},"mcpServers":{"home":{"command":"uvx","args":["home"]}}}"#,
        )
        .expect("write user compat config");
        fs::write(
            home.join("settings.json"),
            r#"{"model":"swarm-standard","env":{"A2":"1"},"hooks":{"PreToolUse":["base"]},"permissions":{"defaultMode":"plan"}}"#,
        )
        ...
        fs::write(
            ...
            r#"{"model":"swarm-opus","permissionMode":"acceptEdits"}"#,
        )
        ...
        assert_eq!(SWARM_SETTINGS_SCHEMA, "SwarmSchema");
        assert_eq!(loaded.loaded_entries().len(), 5);
        assert_eq!(loaded.loaded_entries()[0].source, ConfigSource::User);
        assert_eq!(
            loaded.get("model"),
            Some(&JsonValue::String("swarm-opus".to_string()))
        );
        assert_eq!(loaded.model(), Some("swarm-opus"));
        assert_eq!(
