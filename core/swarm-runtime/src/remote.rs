...
pub const SWARM_REMOTE_BASE_URL: &str = "https://api.clawswarm.dev";
pub const SWARM_SESSION_TOKEN_PATH: &str = "/run/clawswarm/session_token";
pub const SWARM_SYSTEM_CA_BUNDLE: &str = "/etc/ssl/certs/ca-certificates.crt";

pub const SWARM_PROXY_ENV_KEYS: [&str; 8] = [
    "HTTPS_PROXY",
    "https_proxy",
    "NO_PROXY",
    "no_proxy",
...
pub const SWARM_NO_PROXY_HOSTS: [&str; 16] = [
    "localhost",
    "127.0.0.1",
    "::1",
    "169.254.0.0/16",
...
impl RemoteSessionContext {
    ...
    pub fn from_env_map(env_map: &BTreeMap<String, String>) -> Self {
        Self {
            enabled: env_truthy(env_map.get("CLAWSWARM_REMOTE")),
            session_id: env_map
                .get("CLAWSWARM_REMOTE_SESSION_ID")
                .filter(|value| !value.is_empty())
                .cloned(),
            base_url: env_map
                .get("CLAWSWARM_BASE_URL")
                .filter(|value| !value.is_empty())
                .cloned()
                .unwrap_or_else(|| SWARM_REMOTE_BASE_URL.to_string()),
        }
    }
}
...
impl UpstreamProxyBootstrap {
    ...
    pub fn from_env_map(env_map: &BTreeMap<String, String>) -> Self {
        ...
        let token_path = env_map
            ...
            .filter(|value| !value.is_empty())
            .map_or_else(|| PathBuf::from(SWARM_SESSION_TOKEN_PATH), PathBuf::from);
        let system_ca_path = env_map
            .get("CCR_SYSTEM_CA_BUNDLE")
            .filter(|value| !value.is_empty())
            .map_or_else(|| PathBuf::from(SWARM_SYSTEM_CA_BUNDLE), PathBuf::from);
...
pub fn no_proxy_list() -> String {
    let mut hosts = SWARM_NO_PROXY_HOSTS.to_vec();
...
mod tests {
    ...
    fn remote_context_reads_env_state() {
        let env = BTreeMap::from([
            ("CLAWSWARM_REMOTE".to_string(), "true".to_string()),
            (
                "CLAWSWARM_REMOTE_SESSION_ID".to_string(),
                "session-123".to_string(),
            ),
            (
                "CLAWSWARM_BASE_URL".to_string(),
    ...
    fn bootstrap_fails_open_when_token_or_session_is_missing() {
        let env = BTreeMap::from([
            ("CLAWSWARM_REMOTE".to_string(), "1".to_string()),
    ...
    fn helper_outputs_match_expected_shapes() {
        ...
        assert!(no_proxy_list().contains("api.clawswarm.dev"));
