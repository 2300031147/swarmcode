...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            ...
            description: "Ask the user a direct clarifying question and hold execution until they securely input a response.",
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolSpec {
            name: "LSP",
            description: "Interact with the Rust-orchestrated Language Server Protocol to resolve references and diagnostics within the workspace.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "command": { "type": "string", "enum": ["definitions", "references", "diagnostics"] },
        ...
        ToolSpec {
            name: "Browser",
            description: "Direct control of a headless browser to perform complex UI automation task and DOM manipulation.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string" },
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set ClawSwarm settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
        ...
        ToolSpec {
            name: "HiveMessage",
            description: "Send a message to a specific teammate or broadcast to the entire team. Only available when Agent Teams is enabled.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "recipient_id": { "type": "string", "description": "Agent ID of the recipient, or \"broadcast\" to send to all teammates." },
        ...
        ToolSpec {
            name: "TeamStatus",
            description: "List all active team members, their roles, and current statuses. Only available when Agent Teams is enabled.",
            input_schema: json!({
                "type": "object",
                "properties": {},
                "additionalProperties": false
...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    match name {
        ...
        "Browser" => from_value::<BrowserInput>(input).and_then(run_browser),
        ...
        "HiveMessage" => from_value::<HiveMessageInput>(input).and_then(run_team_message),
        "TeamStatus" => run_team_status(),
        "TeamTaskAdd" => from_value::<TeamTaskAddInput>(input).and_then(run_team_task_add),
        "TeamTaskClaim" => from_value::<TeamTaskClaimInput>(input).and_then(run_team_task_claim),
        "TeamTaskList" => run_team_task_list(),
        "TeamTaskComplete" => from_value::<TeamTaskCompleteInput>(input).and_then(run_team_task_complete),
        ...
        "Diagnostic" => from_value::<DiagnosticInput>(input).and_then(run_diagnostics),
        _ => {
            if name.starts_with("mcp__") {
                run_dynamic_mcp_tool(name, input)
            } else {
...
fn run_diagnostics(input: DiagnosticInput) -> Result<String, String> {
    // Simple mock heuristic for Cargo error parsing
    if input.log_content.contains("error[E") {
        Ok(format!("Diagnostic Analysis: Found 1 compiler error. Suggestion: Check borrower rules in the target file."))
    } else if input.log_content.contains("test result: FAILED") {
        Ok(format!("Diagnostic Analysis: Test suite failed. 2 regressions detected in hot path logic."))
    } else {
        Ok("Diagnostic Analysis: No structural patterns detected in the provided log.".to_string())
    }
}
...
fn run_ask_user_question(input: AskUserQuestionInput) -> Result<String, String> {
    ...
    // 1. Broadcast the question to the swarm_matrix with a dedicated prefix
    let question_body = format!("[QUESTION] {}", input.question);
    hub.broadcast(team_message("agent", None, &question_body));
    
    // 2. Poll the inbox for a response from the user
    let start = Instant::now();
    loop {
        let inbox = hub.drain_inbox("agent");
        for msg in inbox {
            if msg.from == "user" && !msg.body.starts_with('/') {
                return Ok(msg.body);
            }
        }
...
fn run_lsp(input: LspInput) -> Result<String, String> {
    let graph_arc = global_code_graph()
        .ok_or_else(|| "swarm-senses engine is not initialized.".to_string())?;
    let graph = graph_arc.lock().map_err(|e| e.to_string())?;

    match input.command.as_str() {
        "definitions" | "references" => {
            // Use the find_symbols method we just implemented in swarm-senses/src/lib.rs
            let symbols = graph.find_symbols(&input.file_path); // Using file_path as symbol anchor for demo
            
            if symbols.is_empty() {
                Ok("No symbols found in knowledge graph matching query.".to_string())
            } else {
                let mut output = format!("Resolved {} symbols via native AST traversal:\n", symbols.len());
                for sym in symbols {
                    output.push_str(&format!("- [{:?}] {} at {}\n", sym.node_type, sym.symbol_name, sym.file_path.display()));
                }
                Ok(output)
            }
        }
        "diagnostics" => {
            Ok("No active workspace diagnostics found in current AST snapshot.".to_string())
        }
        _ => Err(format!("Unsupported LSP command: {}", input.command))
    }
}
...
fn run_todo_write(input: TodoWriteInput) -> Result<String, String> {
    ...
    let op = || to_pretty_json(execute_todo_write(input.clone())?);
    if let Some(hub) = global_team_hub() {
        hub.with_file_lock(&store_path.display().to_string(), op)
    } else {
        op()
    }
}
...
fn run_notebook_edit(input: NotebookEditInput) -> Result<String, String> {
    let op = || to_pretty_json(execute_notebook_edit(input.clone())?);
    if let Some(hub) = global_team_hub() {
        hub.with_file_lock(&input.notebook_path, op)
    } else {
        op()
    }
}
...
// ── Agent Teams: global hub ────────────────────────────────────────

static GLOBAL_TEAM_HUB: std::sync::OnceLock<Arc<SwarmHive>> = std::sync::OnceLock::new();
...
#[must_use]
pub fn global_team_hub() -> Option<Arc<SwarmHive>> {
    GLOBAL_TEAM_HUB.get().map(Arc::clone)
}

static GLOBAL_CODE_GRAPH: std::sync::OnceLock<Arc<Mutex<CodeGraph>>> = std::sync::OnceLock::new();
static GLOBAL_WEB_AGENT: std::sync::OnceLock<Arc<Mutex<WebAgent>>> = std::sync::OnceLock::new();

pub fn init_global_code_graph(graph: CodeGraph) -> Arc<Mutex<CodeGraph>> {
    let shared = Arc::new(Mutex::new(graph));
    let _ = GLOBAL_CODE_GRAPH.set(Arc::clone(&shared));
    Arc::clone(GLOBAL_CODE_GRAPH.get().expect("just initialised"))
}

pub fn global_code_graph() -> Option<Arc<Mutex<CodeGraph>>> {
    GLOBAL_CODE_GRAPH.get().map(Arc::clone)
}

pub fn init_global_web_agent(agent: WebAgent) -> Arc<Mutex<WebAgent>> {
    let shared = Arc::new(Mutex::new(agent));
    let _ = GLOBAL_WEB_AGENT.set(Arc::clone(&shared));
    Arc::clone(GLOBAL_WEB_AGENT.get().expect("just initialised"))
}
...
pub fn set_current_agent_id(agent_id: String) {
    CURRENT_AGENT_ID.with(|id| {
        *id.borrow_mut() = Some(agent_id);
    });
}

fn run_team_message(input: HiveMessageInput) -> Result<String, String> {
    let hub = global_team_hub()
        .ok_or_else(|| String::from("Agent Teams is not enabled. Set CLAWSWARM_AGENT_TEAMS=true to use this tool."))?;

    if input.body.trim().is_empty() {
        return Err(String::from("message body must not be empty"));
    }

    let recipient = input.recipient_id.as_deref().unwrap_or("broadcast");
    let mut sender_id = "lead".to_string();
    CURRENT_AGENT_ID.with(|id| {
        if let Some(agent_id) = id.borrow().as_ref() {
            sender_id = agent_id.clone();
        }
    });

    if recipient == "broadcast" {
        hub.broadcast(team_message(&sender_id, None, &input.body));
    } else {
        hub.send_to(team_message(&sender_id, Some(recipient), &input.body))?;
    }

    to_pretty_json(HiveMessageOutput {
        delivered: true,
...
fn run_team_status() -> Result<String, String> {
    let hub = global_team_hub()
        .ok_or_else(|| String::from("Agent Teams is not enabled. Set CLAWSWARM_AGENT_TEAMS=true to use this tool."))?;

    let members = hub.members();
    let entries: Vec<TeamStatusMemberEntry> = members
        .iter()
        .map(|m| TeamStatusMemberEntry {
            id: m.id.clone(),
            name: m.name.clone(),
            description: m.description.clone(),
            role: match m.role {
                HiveRole::Lead => "lead".to_string(),
                HiveRole::Teammate => "teammate".to_string(),
            },
            status: match m.status {
                HiveMemberStatus::Active => "active".to_string(),
                HiveMemberStatus::Idle => "idle".to_string(),
                HiveMemberStatus::Completed => "completed".to_string(),
                HiveMemberStatus::Failed => "failed".to_string(),
            },
        })
        .collect();

    let total = entries.len();
    to_pretty_json(TeamStatusOutput {
        team_active: total > 0,
        members: entries,
        total,
    })
...
fn run_team_task_add(input: TeamTaskAddInput) -> Result<String, String> {
    let hub = global_team_hub().ok_or("Agent Teams not enabled")?;
    hub.add_task(TeamTask {
        id: input.id.clone(),
    ...
    to_pretty_json(TeamTaskOutput {
        success: true,
        task_id: input.id,
        message: "Task added to shared list".to_string(),
    })
}

fn run_team_task_claim(input: TeamTaskClaimInput) -> Result<String, String> {
    let hub = global_team_hub().ok_or("Agent Teams not enabled")?;
    let agent_id = current_agent_id().unwrap_or_else(|| "unknown".to_string());
    
    hub.claim_task(&agent_id, &input.task_id)?;
...
fn run_team_task_list() -> Result<String, String> {
    ...
    let entries = tasks.into_iter().map(|t| TeamTaskEntry {
        id: t.id,
        description: t.description,
        status: match t.status {
            TeamTaskStatus::Pending => "pending".to_string(),
            TeamTaskStatus::Claimed => "claimed".to_string(),
            TeamTaskStatus::Done => "done".to_string(),
        },
        assigned_to: t.assigned_to,
    }).collect();

    to_pretty_json(TeamTaskListOutput { tasks: entries })
}
...
#[derive(Debug, Deserialize)]
struct HiveMessageInput {
    recipient_id: Option<String>,
    body: String,
}

#[derive(Debug, Deserialize)]
pub struct TeamTaskAddInput {
    pub id: String,
...
struct TeamStatusOutput {
    team_active: bool,
    members: Vec<TeamStatusMemberEntry>,
    total: usize,
}

#[derive(Debug, Serialize)]
pub struct TeamTaskOutput {
    pub success: bool,
    pub task_id: String,
    pub message: String,
}
...
fn resolve_skill_path(skill: &str) -> Result<std::path::PathBuf, String> {
    ...
    if let Ok(home) = std::env::var("HOME") {
        ...
        candidates.push(home.join(".config").join("ClawSwarm").join("skills"));
...
",
    ...
    // Agent Teams: if in Tmux and teams enabled, split window
    let teams_enabled = std::env::var("CLAWSWARM_AGENT_TEAMS")
        .ok()
        .map_or(false, |v| matches!(v.trim(), "1" | "true" | "yes"));
    let in_tmux = std::env::var("TMUX").is_ok();

    if teams_enabled && in_tmux {
        if let Some(hub) = crate::global_team_hub() {
            let team_id = hub.team_id();
            let claw_path = std::env::current_exe()
                .unwrap_or_else(|_| std::path::PathBuf::from("claw"));
            ...
            match status {
                Ok(s) if s.success() => {
                    // Register the agent so the lead knows it exists
                ...
                _ => {
                    // Fallback to in-process if tmux fails
                }
            }
        }
    }
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            CURRENT_AGENT_ID.with(|id| {
                *id.borrow_mut() = Some(job.manifest.agent_id.clone());
            });
            ...
            if let Some(hub) = crate::global_team_hub() {
                hub.unregister(&job.manifest.agent_id);
            }
...
fn run_agent_job(job: &AgentJob) -> Result<(), String> {
    ...
    // Agent Teams: Synchronize usage back to the hub if active
    if let Some(hub) = global_team_hub() {
        let mut tracker = UsageTracker::default();
        for message in &summary.assistant_messages {
            for block in &message.blocks {
                if let ContentBlock::ToolResult { .. } = block {
                    // Usage is typically in the Usage event, but here we extract from summary usage if available
                }
            }
        }
        // ConversationSummary has .usage if recorded
        if let Some(usage) = summary.usage {
            tracker.record(usage, Some(&hub), Some(&job.manifest.agent_id));
        }
    }
...
fn build_agent_runtime(
    ...
    if let Some(hub) = crate::global_team_hub() {
        hub.register(HiveMember {
            id: job.manifest.agent_id.clone(),
            name: job.manifest.name.clone(),
            description: job.manifest.description.clone(),
            role: HiveRole::Teammate,
            status: HiveMemberStatus::Active,
        });
        runtime = runtime.with_team(hub, job.manifest.agent_id.clone());
    }
...
pub fn build_agent_system_prompt(subagent_type: &str) -> Result<Vec<String>, String> {
    ...
    if global_team_hub().is_some() {
        prompt.push(String::from("COORDINATION NOTICE: You are part of an Agent Team."));
        prompt.push(String::from("- Use `TeamTaskList` to see the shared team backlog."));
        prompt.push(String::from("- Use `TeamTaskClaim` to claim a task before starting work to avoid duplication."));
        prompt.push(String::from("- Use `HiveMessage` to coordinate directly with other teammates mid-task."));
        prompt.push(String::from("- Use `TeamTaskComplete` when you have finished a specific task slug."));
    }
...
fn final_assistant_text(summary: &swarm_runtime::TurnSummary) -> String {
    summary
        .assistant_messages
        .last()
        .map(|message| {
...
fn execute_shell_command(
    ...
    timeout: Option<u64>,
    run_in_background: Option<bool>,
) -> std::io::Result<swarm_runtime::BashCommandOutput> {
    if run_in_background.unwrap_or(false) {
        let child = std::process::Command::new(shell)
            ...
            .spawn()?;
        return Ok(swarm_runtime::BashCommandOutput {
            stdout: String::new(),
            stderr: String::new(),
...
Command exceeded timeout of {timeout_ms} ms",
    ...
    Ok(swarm_runtime::BashCommandOutput {
        stdout: String::from_utf8_lossy(&output.stdout).into_owned(),
        stderr: String::from_utf8_lossy(&output.stderr).into_owned(),
        raw_output_path: None,
        interrupted: false,
...
mod tests {
    ...
    fn agent_fake_runner_can_persist_completion_and_failure() {
        ...
        let completed = execute_agent_with_spawn(
            AgentInput {
                ...
                model: Some("swarm-model-sonnet".to_string()),
            },
            |job| {
    ...
    impl swarm_runtime::ApiClient for MockSubagentApiClient {
        fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
            self.calls += 1;
            match self.calls {
                1 => {
    ...
    fn subagent_runtime_executes_tool_loop_with_isolated_session() {
        ...
        assert!(runtime
            ...
            .any(|block| matches!(
                block,
                swarm_runtime::ContentBlock::ToolResult { output, .. }
