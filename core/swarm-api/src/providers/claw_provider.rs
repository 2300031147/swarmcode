...
pub const DEFAULT_BASE_URL: &str = "https://api.clawswarm.dev";
const ClawSwarm_VERSION: &str = "2023-06-01";
...
impl AuthSource {
    pub fn from_env() -> Result<Self, ApiError> {
        let api_key = read_env_non_empty("CLAWSWARM_API_KEY")?;
        let auth_token = read_env_non_empty("CLAWSWARM_AUTH_TOKEN")?;
        match (api_key, auth_token) {
            ...
            (None, None) => Err(ApiError::missing_credentials(
                "Claw",
                &["CLAWSWARM_AUTH_TOKEN", "CLAWSWARM_API_KEY"],
            )),
        }
    }
...
impl ClawApiClient {
    ...
    async fn send_raw_request(
        ...
        let request_builder = self
            .http
            .post(&request_url)
            .header("ClawSwarm-version", ClawSwarm_VERSION)
            .header("content-type", "application/json");
        let mut request_builder = self.auth.apply(request_builder);
        if let Some(agent_id) = &request.agent_id {
            request_builder = request_builder.header("x-claw-agent-id", agent_id);
        }
...
impl AuthSource {
    pub fn from_env_or_saved() -> Result<Self, ApiError> {
        if let Some(api_key) = read_env_non_empty("CLAWSWARM_API_KEY")? {
            return match read_env_non_empty("CLAWSWARM_AUTH_TOKEN")? {
        ...
        if let Some(bearer_token) = read_env_non_empty("CLAWSWARM_AUTH_TOKEN")? {
...
pub fn has_auth_from_env_or_saved() -> Result<bool, ApiError> {
    Ok(read_env_non_empty("CLAWSWARM_API_KEY")?.is_some()
        || read_env_non_empty("CLAWSWARM_AUTH_TOKEN")?.is_some()
...
fn read_auth_token() -> Option<String> {
    read_env_non_empty("CLAWSWARM_AUTH_TOKEN")
        .ok()
...
pub fn read_base_url() -> String {
    std::env::var("CLAWSWARM_BASE_URL").unwrap_or_else(|_| DEFAULT_BASE_URL.to_string())
}
...
mod tests {
    ...
    use swarm_runtime::{clear_oauth_credentials, save_oauth_credentials, OAuthConfig};
    ...
    fn read_api_key_requires_presence() {
        let _guard = env_lock();
        std::env::remove_var("CLAWSWARM_AUTH_TOKEN");
        std::env::remove_var("CLAWSWARM_API_KEY");
    ...
    fn read_api_key_requires_non_empty_value() {
        let _guard = env_lock();
        std::env::set_var("CLAWSWARM_AUTH_TOKEN", "");
        std::env::remove_var("CLAWSWARM_API_KEY");
    ...
    fn read_api_key_prefers_api_key_env() {
        let _guard = env_lock();
        std::env::set_var("CLAWSWARM_AUTH_TOKEN", "auth-token");
        std::env::set_var("CLAWSWARM_API_KEY", "legacy-key");
        assert_eq!(
    ...
    fn resolve_saved_oauth_token_refreshes_expired_credentials() {
        ...
        let stored = swarm_runtime::load_oauth_credentials()
