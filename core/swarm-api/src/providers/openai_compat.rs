...
pub const DEFAULT_CLAWSWARM_XAI_BASE_URL: &str = "https://api.x.ai/v1";
...
pub struct OpenAiCompatConfig {
    ...
    /// All env vars that can satisfy auth for this provider (used in error messages)
    pub auth_env_vars: &'static [&'static str],
}

impl OpenAiCompatConfig {
    #[must_use]
    pub const fn ClawSwarm() -> Self {
        Self {
            provider_name: "ClawSwarm",
            api_key_env: "CLAWSWARM_XAI_KEY",
            base_url_env: "CLAWSWARM_XAI_BASE_URL",
            default_base_url: DEFAULT_CLAWSWARM_XAI_BASE_URL,
            auth_env_vars: &["CLAWSWARM_XAI_KEY"],
        }
    }
    ...
    /// Build a client that uses a dummy API key (for local servers that ignore auth).
    #[must_use]
    pub fn into_keyless_client(self) -> OpenAiCompatClient {
        OpenAiCompatClient::new("clawswarm-local", self)
    }
}
...
impl OpenAiCompatClient {
    ...
    async fn send_raw_request(
        ...
        if let Some(agent_id) = &request.agent_id {
            builder = builder.header("X-Claw-Agent-ID", agent_id);
        }
...
mod tests {
    ...
    #[test]
    fn missing_CLAWSWARM_XAI_KEY_is_provider_specific() {
        let _lock = env_lock();
        std::env::remove_var("CLAWSWARM_XAI_KEY");
        let error = OpenAiCompatClient::from_env(OpenAiCompatConfig::ClawSwarm())
