...
async fn send_message_uses_openai_compatible_endpoint_and_auth() {
    ...
    let body = concat!(
        "{",
        "\"id\":\"chatcmpl_test\",",
        "\"model\":\"swarm-model-frontier\",",
        "\"choices\":[{",
        "\"message\":{\"role\":\"assistant\",\"content\":\"Hello from Swarm\",\"tool_calls\":[]},",
        "\"finish_reason\":\"stop\"",
        "}],",
    ...
    let client = OpenAiCompatClient::new("clawswarm-test-key", OpenAiCompatConfig::ClawSwarm())
    ...
    assert_eq!(response.model, "swarm-model-frontier");
    assert_eq!(response.total_tokens(), 16);
    assert_eq!(
        response.content,
        vec![OutputContentBlock::Text {
            text: "Hello from Swarm".to_string(),
        }]
    );
    ...
    assert_eq!(
        ...
        Some("Bearer ClawSwarm-test-key")
    );
    let body: serde_json::Value = serde_json::from_str(&request.body).expect("json body");
    assert_eq!(body["model"], json!("swarm-model-frontier"));
...
async fn stream_message_normalizes_text_and_multiple_tool_calls() {
    let state = Arc::new(Mutex::new(Vec::<CapturedRequest>::new()));
    let sse = concat!(
        "data: {\"id\":\"chatcmpl_stream\",\"model\":\"swarm-model-frontier\",\"choices\":[{\"delta\":{\"content\":\"Hello\"}}]}\n\n",
    ...
    assert_eq!(stream.request_id(), Some("req_swarm_stream"));
...
async fn provider_client_dispatches_xai_requests_from_env() {
    let _lock = env_lock();
    let _api_key = ScopedEnvVar::set("CLAWSWARM_XAI_KEY", "clawswarm-test-key");
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_provider\",\"model\":\"swarm-model-frontier\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Through provider client\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":9,\"completion_tokens\":4}}",
        )],
    )
    .await;
    let _base_url = ScopedEnvVar::set("CLAWSWARM_XAI_BASE_URL", server.base_url());

    let client =
        ProviderClient::from_model("swarm-model-frontier").expect("ClawSwarm provider client should be constructed");
    assert!(matches!(client, ProviderClient::ClawSwarm(_)));
...
fn sample_request(stream: bool) -> MessageRequest {
    MessageRequest {
        model: "swarm-model-frontier".to_string(),
        max_tokens: 64,
