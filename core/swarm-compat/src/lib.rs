...
use swarm_commands::{CommandManifestEntry, CommandRegistry, CommandSource};
use swarm_runtime::{BootstrapPhase, BootstrapPlan};
use swarm_tools::{ToolManifestEntry, ToolRegistry, ToolSource};
...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    if let Some(explicit) = std::env::var_os("CLAWSWARM_UPSTREAM") {
        candidates.push(PathBuf::from(explicit));
    }

    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("clawswarm"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("clawswarm"));
    candidates.push(primary_repo_root.join("vendor").join("clawswarm"));
...
pub fn extract_bootstrap_plan(source: &str) -> BootstrapPlan {
    ...
    if source.contains("--clawswarm-browser-mcp") {
